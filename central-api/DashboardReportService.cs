...
internal sealed class DashboardReportService(NpgsqlDataSource dataSource, ApiOptions options)
{
        ...
        DashboardUser user, string branchCode, DateOnly? oldestDate, CancellationToken ct)
    {
        await using var command = dataSource.CreateCommand("""
            ...
            WHERE b.active = true AND b.code = $1
              ...
              AND ($3::timestamp IS NULL OR s.closed_at IS NULL OR s.opened_at >= $3)
        ...
        command.Parameters.AddWithValue(user.Id);
        command.Parameters.AddWithValue((object?)oldestDate?.ToDateTime(TimeOnly.MinValue) ?? DBNull.Value);
        await using var reader = await command.ExecuteReaderAsync(ct);
        ...
        CancellationToken ct)
    {
        ...
        await using (var command = dataSource.CreateCommand("""
            ...
            WHERE branch_id = $1 AND source_folio = $2
              AND ($3::timestamp IS NULL OR business_date >= $3)
    ...
    public async Task<ProductCancellationReport?> GetProductCancellationReportAsync(DashboardUser user, string branchCode, DateOnly from, DateOnly to, int? shiftId, string? userFilter, string? productFilter, int page, int pageSize, CancellationToken ct)
    {
        ...
        // El selector entrega shifts.source_shift_id (idturnointerno), pero las
        // cancelaciones conservan cheques.idturno. GetMetaAsync ya resuelve el
        // número operativo del turno seleccionado; usarlo evita excluir eventos
        // válidos al filtrar, por ejemplo, 610300 frente a 425.
        var reportShiftId = shiftId is null ? null : meta.ShiftNumber;
